...
def main(source_url: str = None, target_url: str = None):
    ...
    for idx, (_, row) in enumerate(all_rows):
        ...
        if not query_text:
            logger.warning(f"第{idx+1}行：必过集query为空，跳过")
            continue

        if not function_text:
            logger.warning(f"第{idx+1}行：glasses_function为空，跳过")
            continue

        # 拆分query（使用LLM智能拆分）
        queries = split_queries_with_llm(query_text, logger)
        logger.info(f"第{idx+1}行：拆分出{len(queries)}个query，function={function_text[:50]}...")
